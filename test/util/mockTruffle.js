...
/*
  This file contains utilities for generating a mock truffle project to test solprofiler's
  run script against.
 */
const fs = require('fs');
...
module.exports.install = function install(contract, test, config, _trufflejs, _trufflejsName) {
  ...
  fs.writeFileSync('./.solprofiler.js', configjs);
 ...
 * and test specified by the params.
 * @param  {config} .solprofiler.js configuration
 */
...
/**
 * Removes mock truffle project and profiler reports generated by exec.js
 */
module.exports.remove = function remove() {
  shell.config.silent = true;
  shell.rm('./.solprofiler.js');
  shell.rm('-Rf', 'mock');
  shell.rm('-Rf', 'profiler');
  shell.rm('profiler.json');
