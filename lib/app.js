...
const ProfilerMap = require('./profilerMap.js');
...
class App {
  constructor(config) {
    this.profilerDir = './profilerEnv';          // Env that instrumented .sols are tested in
    ...
    this.profiler = new ProfilerMap();           // initialize a profiler map
    ...
    this.copyNodeModules = config.copyNodeModules || false;  // Copy node modules into profilerEnv?
    this.copyPackages = config.copyPackages || [];           // Only copy specific node_modules packages into profilerEnv
  ...
  /**
   * Generates a copy of the target project configured for solidity-profiler and saves to
   * the profiler environment folder. Process exits(1) if try fails
  */
  generateProfilerEnvironment() {
    this.log('Generating profiler environment');
    ...
    try {
      ...
      shell.mkdir(this.profilerDir);
      shell.cp('-R', files, this.profilerDir);

      // Add specific node_modules packages.
      if (!this.copyNodeModules && this.copyPackages.length) {
        shell.mkdir(this.profilerDir + '/node_modules');
        this.copyPackages.forEach((nodePackage) => {
          shell.mkdir('-p', this.profilerDir + '/node_modules/' + nodePackage);
          shell.cp('-rfL', 'node_modules/' + nodePackage + '/.', this.profilerDir + '/node_modules/' + nodePackage);
        });
      }
      ...
      // Profiler network opts specified: use port if declared
      if (truffleConfig && truffleConfig.networks && truffleConfig.networks.profiler) {
        this.port = truffleConfig.networks.profiler.port || this.port;
        this.network = '--network profiler';

      // No profiler network defaults to the dev network on port 8555, high gas / low price.
      } else {
        const trufflejs = defaultTruffleConfig(this.port, gasLimitHex, gasPriceHex);

        (process.platform === 'win32')
          ? fs.writeFileSync(`${this.profilerDir}/truffle-config.js`, trufflejs)
          : fs.writeFileSync(`${this.profilerDir}/truffle.js`, trufflejs);
      }
      ...
      if (shell.test('-e', `${this.profilerDir}${this.buildDirPath}`)){
        shell.rm('-Rf', `${this.profilerDir}${this.buildDirPath}`)
      }
      ...
      const msg = ('There was a problem generating the profiler environment: ');
   ...
   * + Generate file path reference for profiler report
   * + Load contract as string
   * + Instrument contract
   * + Save instrumented contract in the profiler environment folder where covered tests will run
   * + Add instrumentation info to the profiler map
   */
  instrumentTarget() {
    this.skipFiles = this.skipFiles.map(contract => `${this.profilerDir}/contracts/${contract}`);
    this.skipFiles.push(`${this.profilerDir}/contracts/Migrations.sol`);

    const instrumentedFiles = [];
    let currentFile;
    try {
      shell.ls(`${this.profilerDir}/contracts/**/*.sol`).forEach(file => {
        currentFile = file;
        if (!this.skipFiles.includes(file) && !this.inSkippedFolder(file)) {
          ...
          const canonicalPath = contractPath.split('/profilerEnv').join(working);
          ...
          this.profiler.addContract(instrumentedContractInfo, canonicalPath);
   ...
   *  profiler environment folder. Shell cd command needs to be invoked
  ...
  runTestCommand() {
    try {
      ...
      const msg =
      `
        There was an error generating profiler. Possible reasons include:
  ...
  loadArtifacts() {
    ...
    shell.ls(`${this.profilerDir}${this.buildDirPath}/*.json`).forEach(file => {
   ...
   */
  modifyArtifacts(){
    shell.ls(`${this.profilerDir}${this.buildDirPath}/*.json`).forEach((file, index) => {
      const artifactPath = this.platformNeutralPath(file);
      const artifactRaw = fs.readFileSync(artifactPath);
      const artifact = JSON.parse(artifactRaw);
  ...
  /**
   * Generate profiler / write profiler report / run istanbul
   */
  generateReport() {
    ...
    return new Promise((resolve, reject) => {
      ...
      reader
        ...
        .on('close', () => {
          ...
          try {
            this.profiler.generate(this.events, `${this.workingDir}/contracts`);
            const relativeMapping = this.makeKeysRelative(this.profiler.profiler, this.workingDir);
            const json = JSON.stringify(relativeMapping);
            fs.writeFileSync('./profiler.json', json);
            ...
            reporter.write(collector, true, () => {
              this.log('Istanbul profiler reports generated');
              this.cleanUp();
              resolve();
            });
          } catch (err) {
            const msg = 'There was a problem generating the profiler map / running Istanbul.\n';
  ...
  sanityCheckContext(){
    ...
    if (shell.test('-e', `${this.workingDir}/${this.profilerDir}`)){
      shell.rm('-Rf', this.profilerDir);
    }

    if (shell.test('-e', `${this.workingDir}/spTopics`)){
      shell.rm(`${this.workingDir}/spTopics`);
    }
  }

  /**
   * Relativizes path keys so that istanbul report can be read on Windows
   * @param  {Object} map  profiler map generated by profilerMap
  ...
  makeKeysRelative(map, root) {
    ...
    Object.keys(map).forEach(pathKey => {
      newProfiler[path.relative(root, pathKey)] = map[pathKey];
    });
    return newProfiler;
  }
  ...
  inSkippedFolder(file){
    ...
    this.skippedFolders.forEach(folderToSkip => {
      folderToSkip = `${this.profilerDir}/contracts/${folderToSkip}`;
  ...
  /**
   * Removes profiler build artifacts, kills testrpc.
  ...
  cleanUp(err) {
    ...
    function exit(err){
      if (err) {
        self.log(`${err}\nExiting without generating profiler...`);
        process.exit(1);
      } else if (self.testsErrored) {
        self.log('Some truffle tests failed while running profiler');
        process.exit(1);
      } else {
        self.log('Done.');
        process.exit(0);
    ...
    self.log('Cleaning up...[NOR REALLY]');
    shell.config.silent = true;
    //shell.rm('-Rf', self.profilerDir);
